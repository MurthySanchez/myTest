...
int UI(char* name){
    ...
    gtk_table_attach_defaults(GTK_TABLE(table),entryUser,12,19,9,10);
    /*********************/
    ...
    gtk_table_attach_defaults(GTK_TABLE(table),entryPW,12,19,11,12);

    image=gtk_image_new_from_file("./image/eye.png");
    check=gtk_check_button_new();
    gtk_container_add(GTK_CONTAINER(check),image);
    gtk_table_attach_defaults(GTK_TABLE(table),check,19,21,11,12);
