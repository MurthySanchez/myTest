...
{
    new_row[0] = gtk_entry_get_text(GTK_ENTRY(entry_user_id));
    new_row[1] = gtk_entry_get_text(GTK_ENTRY(entry_user_name));
    new_row[2] = gtk_entry_get_text(GTK_ENTRY(entry_user_sex));
    new_row[3] = gtk_entry_get_text(GTK_ENTRY(entry_user_age));
...
void append_row(GtkWidget *button, gint data)
{
    ...
    {
    case 0:
        add_win = create_addwin_user(window_a); //admin添加员工信息
        break;
    case 1:
        add_win = create_addwin_attend(window_a); //admin员工考勤信息
        break;
    case 2:
        add_win = create_addwin_user_attend(window_a); //员工查看自身考勤信息
        break;
    default:
        g_print("error.\n");
        break;
...
GtkWidget *admin_functionTwo(GtkWidget *window, gchar **titles, int field, int length, gint id)
{
    ...
    case 2:
        ...
        break;
    default:
        g_print("error in admin_functionTwo(),create clist.\n");
        break;
    }
...
GtkWidget *create_addwin_user(GtkWidget *window)
{
    ...
    gtk_table_attach_defaults(GTK_TABLE(table), entry_user_id, 1, 2, 0, 1);
    label = gtk_label_new("姓名");
    gtk_table_attach_defaults(GTK_TABLE(table), label, 0, 1, 1, 2);
    entry_user_name = gtk_entry_new();
    gtk_table_attach_defaults(GTK_TABLE(table), entry_user_name, 1, 2, 1, 2);
    label = gtk_label_new("性别");
    gtk_table_attach_defaults(GTK_TABLE(table), label, 0, 1, 2, 3);
    entry_user_sex = gtk_entry_new();
    gtk_table_attach_defaults(GTK_TABLE(table), entry_user_sex, 1, 2, 2, 3);
    label = gtk_label_new("年龄");
    gtk_table_attach_defaults(GTK_TABLE(table), label, 0, 1, 3, 4);
    entry_user_age = gtk_entry_new();
    gtk_table_attach_defaults(GTK_TABLE(table), entry_user_age, 1, 2, 3, 4);
    bbox = gtk_hbutton_box_new();
    gtk_box_pack_start(GTK_BOX(vbox), bbox, FALSE, FALSE, 5);
    gtk_box_set_spacing(GTK_BOX(bbox), 5);
